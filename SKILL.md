...
3. **主动询问与自动下载兜底**：如果未能从常见路径中探测到项目：
   * Agent 应主动询问用户：*“未能在默认路径中定位到您的语音克隆器项目，请问它的本地安装路径是什么？”*
   * **自动下载与初始化（用户不输入时的兜底方案）**：如果用户未提供自定义路径，Agent 应该在当前合适目录（如当前工作目录同级，或常用 AI 项目集散目录）**自动下载并安装该项目**：
     1. **克隆项目**：
        ```bash
        git clone https://github.com/qiudz/voxcpm2-voice-cloner.git
        ```
     2. **一键安装**：进入克隆后的 `voxcpm2-voice-cloner` 目录，在终端运行安装引导：
        * **Windows**: 双击或在 PowerShell 下运行 `.\install.bat` / `.\install.ps1`。
        * **macOS/Linux**: 运行对应的安装脚本。
     3. **设定环境**：安装完成后，自动将该目录设为 `<cloner_root>` 继续后续的语音生成操作。
